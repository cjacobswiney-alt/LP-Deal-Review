...
- CONGRUENCE: Your analysis must be internally consistent across all sections. If you describe leverage as tight in one section, do not call it conservative in another. If you flag a concern, do not list the same item as a strength elsewhere. Review your full output for contradictions before finalizing.`;
...
- CRITICAL: Every numbered item MUST be an actual question ending in "?" — do NOT include statements, observations, or commentary as numbered items. Instead of "This is aggressive" write "What assumptions support this projection?"
...
Calculate net sponsor exposure (co-invest minus one-time transaction fees paid at closing only — acquisition fee, disposition fee). NEVER net asset management fees against co-invest under any circumstances — asset management fees are ongoing recurring fees paid over the life of the deal, not upfront capital extractions, regardless of when or how they are paid. Asset management fees at 2% are market standard — only flag if they exceed 2%. Property management fees paid to a third-party manager are a standard operating expense — do NOT combine them with GP fees, do NOT flag them unless they are wildly above market (8%+), and do NOT add them to any GP fee total. Co-invest benchmarks: 5-10% of total equity is satisfactory. Over 10% is a major positive and should be highlighted as strong GP alignment — 20%+ is exceptional and demonstrates significant skin in the game. 1-2% acquisition fee is market. 20-30% promote is market. For development deals: if developer fee + affiliated GC fee exceeds co-invest, note it.
...
Evaluate leverage, rate type, maturity alignment, and DSCR. Do NOT describe leverage as "conservative" if DSCR is below 1.20x — sub-1.20x DSCR makes a deal thin regardless of LTV or LTC. High LTV (75%+) combined with sub-1.20x DSCR should be noted as tight, not conservative. If the loan is not committed, tell the LP to request the executed term sheet before closing. Calculate the impact of a 25-50bps rate increase if floating (rates are already elevated — 150-200bps stress tests are unrealistic in the current environment).
